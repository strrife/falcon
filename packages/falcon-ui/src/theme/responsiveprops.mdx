...
import { ResponsivePropsTable, ThemeBreakpoints } from 'docs';
...
```
All of `@deity/falcon-ui` components come with default theme which can be overriden as shown in example above.
...
Simply pass object as responsive prop value with it's keys being breakpoints keys defined in `Theme` and values being property values to have responsive prop that changes based on 
screen width. 
