...
Every component has default theme build-in, providing consistent and pleasing look and feel out of the box, yet giving possibility to customize it in every detail.
...
## Customizing theme
All theme customizations are done using `createTheme` function by providing necessary overrides and passing it's output as `theme` prop to `ThemeProvider`.
...
## Custom themed components
  In addition to build-in components there is an easy way to create custom components with default theme that also plugin-in into general theming system.
